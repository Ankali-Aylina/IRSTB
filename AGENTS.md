...
| Section | 使用方    | 键值                                                                                                                                 |
| ------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `"TC"`  | TCCore    | `InitStatus`, `DataTransmissionDelay`, `InitCpuTemp`, `InitGpuTemp`, `CpuStep`, `GpuStep`, `WarningCpu`, `WarningGpu`, `HistorySize` |
| `"BLE"` | BLEThread | `Init`, `TargetName`, `TargetServiceUUID`, `TargetCharacteristicUUID`, `TargetID`                                                    |
| `"UI"`  | TCV3      | `InitStatus`, `dataTxDelay`, `AutoStart`                                                                                             |

每个段使用 `InitStatus` 作为"段已初始化"标记（通过 `isInitialized()` 检查）。
...
9. **vcxproj 文件分类**：`QObject` 子类头文件必须标为 `<QtMoc>`（非 `<ClInclude>`），否则 MOC 不生成元对象代码导致链接错误；`.ui` 文件必须标为 `<QtUic>`；`.qrc` 文件标为 `<QtRcc>`
10. **DLL 初始化必须在主线程**：`inteltemp_initialize` 等 DLL init 函数必须在主线程调用（`TCCore::initialize()` 中），不要移到工作线程
11. **`controlDataUpdated(char*)` 传递静态缓冲区指针**——跨线程 `QueuedConnection` 安全（数据拷贝），但绝不能改为 `DirectConnection`
12. **`stop()` 后必须 `moveToThread(QThread::currentThread())`**——在将线程指针置空之前，否则析构时出现 QObject 父子关系问题
13. **`IniManagement` 存在两个实例**：TCV3 拥有的（用于配置访问）和静态单例（仅用于日志连接），不要混淆
14. **C++/WinRT NuGet 包已导入但未使用**——不要添加 `.idl` 文件，除非理解这是有意保留的
